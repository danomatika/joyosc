...
class DBus
{
    public:
        ...
        int setupSession(char *name);
        int setupSystem(char *name);
        ...
        int createSignal(char *obj, char *iface, char *name);

        int sendSignal();

        /* Signal Listening */

        int addMatchRule(char *rule);

        int removeMatchRule(char *rule);

        bool readSignal();

        bool isSignal(char *iface, char *name);
